...
class RepositorioLibro:
    ...
    def eliminar_libro_isbn(self, isbn:str)->bool:
        ...
        verificarSiLibroPrestamo = self.__repo_prestamos.obtenerTodos()

        for p in verificarSiLibroPrestamo:
            if p.obtenerISBN() == isbn:
                raise Exception("El libro no se puede eliminar porque tiene un prestamo relacionado")
