...
class RepositorioSocios:
    ...
    def eliminar_socio(self, dni:int)->bool:
        ...
        self.__repo_prestamos = RepositorioPrestamos()
        if not isinstance(dni, int) or dni < 0:
            raise ValueError("El dni a buscar debe ser entero valido")
        

        verificarSiEstaEnPrestamo = self.__repo_prestamos.obtenerTodos()
        for p in verificarSiEstaEnPrestamo:
            if p.obtenerSocioDNI() == dni:
                raise Exception("El socio no puede eliminarse porque tiene un prestamos")
